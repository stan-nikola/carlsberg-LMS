...
// Картка курсу (components/CourseTile.jsx) і сторінка курсу
// (app/courses/[slug]/page.js) мають вирішувати "чи можна зараз
// перепройти" ОДНАКОВО. 2026-09-19 вони розійшлись: картка дивилась лише
// на бал курсу і писала «Покращити результат», а сторінка рахувала паузу
// перепроходження по модулях і відкривала методичку. Ці тести тримають
// обидва боки на одному розрахунку.
describe("getModuleStatusList — чи можна перепройти ЗАРАЗ (підпис кнопки на картці)", () => {
  const course = {
    retryFreeAttempts: null,
    retryCooldownHours: null,
    modules: [
      { id: 1, title: "m1", order: 1, cooldownDays: null, retakeCooldownDays: null, screens: [] },
      { id: 2, title: "m2", order: 2, cooldownDays: null, retakeCooldownDays: null, screens: [] },
    ],
  };
  const completedAt = new Date("2026-09-18T10:00:00Z");
  const completions = [
    { moduleId: 1, passed: true, scorePercent: 83, completedAt },
    { moduleId: 2, passed: true, scorePercent: 100, completedAt },
  ];

  it("складений нижче 100% модуль під паузою — перепройти не можна, але видно, коли буде можна", async () => {
    const { getModuleStatusList } = await import("@/lib/courseContent");
    const mods = getModuleStatusList(course, completions, new Date("2026-09-19T06:00:00Z"));

    expect(mods[0].canRetakeNow).toBe(false);
    expect(mods[0].retakeAvailableAt).toEqual(new Date("2026-09-20T10:00:00Z"));
  });

  it("та сама ситуація після паузи — перепройти можна, дати чекання вже нема", async () => {
    const { getModuleStatusList } = await import("@/lib/courseContent");
    const mods = getModuleStatusList(course, completions, new Date("2026-09-21T06:00:00Z"));

    expect(mods[0].canRetakeNow).toBe(true);
    expect(mods[0].retakeAvailableAt).toBeNull();
  });

  it("складений на 100% не перепроходиться ніколи — і дати чекання не показує", async () => {
    const { getModuleStatusList } = await import("@/lib/courseContent");
    const mods = getModuleStatusList(course, completions, new Date("2026-09-30T06:00:00Z"));

    expect(mods[1].canRetakeNow).toBe(false);
    expect(mods[1].retakeAvailableAt).toBeNull();
  });

  it("той самий вердикт, що й у getSessionModules — картка не може розійтись зі сторінкою", async () => {
    const { getModuleStatusList, getSessionModules } = await import("@/lib/courseContent");
    const byId = new Map(completions.map((c) => [c.moduleId, c]));

    for (const now of [new Date("2026-09-19T06:00:00Z"), new Date("2026-09-21T06:00:00Z")]) {
      const mods = getModuleStatusList(course, completions, now);
      const cardSaysImprovable = mods.some(
        (m) => m.status === "completed" && m.scorePercent !== null && m.scorePercent < 100 && m.canRetakeNow
      );
      const pageOpensPlayer = getSessionModules(course, byId, now).playable.length > 0;

      expect(cardSaysImprovable).toBe(pageOpensPlayer);
    }
  });
});
