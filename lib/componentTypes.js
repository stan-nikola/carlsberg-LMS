...
export const COMPONENT_TYPES = [
  ...
  // Значення в enum лишається "photo" (у базі вже є такі компоненти) —
  // змінилась лише назва: сюди тепер кладуть і короткий ролик.
  { value: "photo", label: "фото та відео", hint: "Фото або відео з YouTube/Vimeo за посиланням, з рубрикою і підписом; гейта немає" },
  ...
  { value: "imagepins", label: "фото з точками", hint: "Пояснити частини фото: підписи видно одразу; далі — коли торкнулись кожної точки" },
  { value: "beforeafter", label: "до / після", hint: "Два фото в одній рамці; далі — коли перемкнули" },
];
...
export function defaultContentForType(type) {
  switch (type) {
    ...
    case "photo":
      // Ті самі текстові поля, що в info: рубрика — заголовок — вступний
      // рядок — медіа — «Варто знати». Без них екран із самим лише фото
      // читався як загублений слайд, і автор заводив поруч окремий
      // інфо-екран тільки заради підпису.
      return { kicker: "", lead: "", note: "", images: [] };
    case "imagepins":
      // Точки — у ВІДСОТКАХ кадру, як і зони hotspot: те саме фото
      // показується на телефоні й на ноутбуці різного розміру.
      // Форма й розмір — ОДНІ на весь компонент (рішення користувача
      // 2026-09-23): головна вимога — щоб маркери на знімку були однакові,
      // а окремий розмір у кожної точки саме це й ламає.
      return { kicker: "", lead: "", images: [], pins: [], pinShape: "circle", pinSize: 8 };
    case "beforeafter":
      // images[0] — «до», images[1] — «після». Окремих полів під них немає
      // навмисно: порядок задають ті самі стрілки, що й усюди.
      return { kicker: "", lead: "", images: [], beforeLabel: "Було", afterLabel: "Стало" };
    case "ordering":
...
export function gateTotal(component) {
  ...
  switch (component?.type) {
    ...
    case "input":
      return 1;
    case "imagepins":
      return (c.pins || []).length;
    case "beforeafter":
      // Достатньо ОДНОГО перемикання: сенс екрана — побачити різницю, а не
      // клацати туди-сюди.
      return 1;
    default:
      return 0;
...
export function gateHint(component) {
  ...
  switch (component?.type) {
    ...
    case "imagepins":
      return "Торкніться кожної точки на фото";
    case "beforeafter":
      return "Погляньте, як стало";
    default:
      return "";
  }
}
