...
export function rankHardestModules(rows, limit = HARD_MODULES_LIMIT) {
  ...
  for (const mc of rows) {
    const entry = byModule.get(mc.moduleId) || {
      ...
      courseSlug: mc.module.course?.slug || null,
      total: 0,
      failed: 0,
      attempts: 0,
    };
...
export async function getDashboardStats(employeeIds) {
  ...
  let completed = 0;
  let overdue = 0;
  // Призначення, де вже є рух (не "не розпочато") — четверте кільце
  // "Показників команди". Рахує САМЕ ПРИЗНАЧЕННЯ, як і три сусідніх:
  // доти воно рахувало ЛЮДЕЙ (engagementRate), і "Розпочали 100%"
  // сусідило з "1 курс не розпочато" у списку уваги — та сама картка
  // змішувала дві різні одиниці (скарга користувача, 2026-09-23).
  let started = 0;
  let scoreSum = 0;
  let scoreCount = 0;
  let passedCount = 0;
  ...
  for (const e of enrollments) {
    ...
    if (e.status !== "not_started") {
      engagedEmployeeIds.add(e.employeeId);
      started += 1;
    }

    const courseEntry = byCourse.get(e.course.id) || { id: e.course.id, slug: e.course.slug, title: e.course.title, total: 0, completed: 0 };
    courseEntry.total += 1;
  ...
  const courseBreakdown = Array.from(byCourse.values())
    .map((c) => ({ id: c.id, slug: c.slug, title: c.title, total: c.total, completed: c.completed, pct: c.total ? Math.round((c.completed / c.total) * 100) : 0 }))
  ...
  return {
    ...
    // Частка ПРИЗНАЧЕНЬ, у яких є рух (кільце "Розпочато").
    startedRate: enrollments.length > 0 ? Math.round((started / enrollments.length) * 100) : 0,
    notStartedCount: enrollments.length - started,
    // Залученість — частка КОМАНДИ, що хоч якось почала бодай один курс.
    // Лишається для Excel-звіту; на дашборді її роль виконує полоса
    // статусів (сегмент "Не почали"), а кільце тепер рахує призначення.
...
export async function getWeeklyTrend(employeeIds) {
  ...
  const completions = await prisma.moduleCompletion.findMany({
    ...
    select: { completedAt: true, enrollment: { select: { employee: { select: { id: true, name: true } } } } },
  });
  ...
  for (const c of completions) {
    ...
    if (weekIndex >= 0 && weekIndex < TREND_WEEKS) {
      ...
      const { id, name } = c.enrollment.employee;
      const people = peopleBuckets[bucketIndex];
      const prev = people.get(id) || { name, count: 0 };
      people.set(id, { name, count: prev.count + 1 });
    }
  }
  ...
  return buckets.map((count, i) => ({
    label: i === TREND_WEEKS - 1 ? "Цей тиждень" : `-${TREND_WEEKS - 1 - i} тиж.`,
    count,
    // weekIndex для drill-down (/manager/team?week=N): 0 — цей тиждень.
    weekIndex: TREND_WEEKS - 1 - i,
    people: Array.from(peopleBuckets[i], ([id, p]) => ({ id, name: p.name, count: p.count })).sort((a, b) => b.count - a.count),
  }));
}
