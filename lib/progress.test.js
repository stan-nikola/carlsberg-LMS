...
import { isRecentlyAssigned, isOverdue, pickContinueEnrollments, groupLearning } from "@/lib/progress";
...
describe("groupLearning", () => {
  it("обов'язкові: прострочені → за дедлайном; решта: розпочаті → за дедлайном; пройдені окремо", () => {
    const now = new Date("2026-09-15");
    const es = [
      { id: 1, isMandatory: true, status: "not_started", dueDate: "2026-09-30" },
      { id: 2, isMandatory: true, status: "in_progress", dueDate: "2026-09-01" }, // прострочено
      { id: 3, isMandatory: true, status: "not_started", dueDate: "2026-09-20" },
      { id: 4, isMandatory: false, status: "not_started", dueDate: null },
      { id: 5, isMandatory: false, status: "in_progress", dueDate: null },
      { id: 6, isMandatory: false, status: "not_started", dueDate: "2026-10-01" },
      { id: 7, isMandatory: true, status: "completed", dueDate: "2026-09-01" },
    ];
    const g = groupLearning(es, now);
    expect(g.mandatory.map((e) => e.id)).toEqual([2, 3, 1]);
