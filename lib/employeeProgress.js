 ...
 *
 * "use cache" (не unstable_cache — той не рахується межею кешу для
 * Cache Components-валідатора "instant navigation": дев-лог продовжував
 * кричати blocking-prerender-dynamic саме на цьому виклику навіть після
 * обгортання в unstable_cache, доки не перейшли на директиву; офіційний
 * migration guide прямо каже "unstable_cache is replaced by use cache").
 * Ключ кешу — самі аргументи (employeeId), окремого масиву-префіксу не
 * треба.
 */
export async function getEmployeeEnrollments(employeeId) {
  "use cache";
  cacheLife("minutes"); // той самий орієнтир, що й старий revalidate:60
