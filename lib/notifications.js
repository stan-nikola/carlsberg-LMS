...
export async function notifyBadgeAwarded(awards) {
  ...
  // шляху/query (2026-09-22: підсвітка відзнаки для керівника губилась
  // саме тут). URL має вести на /manager/achievements для них, на
  // /hub/achievements — для решти.
  const allIds = Array.from(new Set(awards.map((a) => a.employeeId)));
