...
import { categoryOf, wantsCategory } from "@/lib/notificationTypes";
...
export async function notifyEmployees(employeeIds, event) {
  ...
  return { created: created.length, pushed: sent };
}
 ...
 */
export async function sendBroadcast({ title, message, url, target }) {
  ...
  const broadcast = await prisma.broadcast.create({
    data: { title, message, url: url || null, targetSummary: describeTarget(target), sentCount: 0 },
  });
  const res = await notifyEmployees(
    employees.map((e) => e.id),
    { type: "broadcast", title, message, url: url || null, dedupeKey: (id) => `broadcast:${broadcast.id}:${id}` }
  );
  await prisma.broadcast.update({ where: { id: broadcast.id }, data: { sentCount: res.created } });
  return { broadcastId: broadcast.id, recipients: employees.length, created: res.created, pushed: res.pushed };
}
