...
export async function notifyEmployees(employeeIds, event) {
  ...
  // Telegram — тим, хто прив’язав чат і не вимкнув канал у профілі
  // (NotificationPreference.telegram; рядка нема = увімкнено).
  let telegram = 0;
  if (channels.telegram) {
    const tgIds = createdIds.filter((id) => prefById.get(id)?.telegram !== false);
    if (tgIds.length > 0) {
