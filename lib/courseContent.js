  ...
  for (let i = 0; i < course.modules.length; i += 1) {
    ...
    if (i === 0) available = true;
      ...
      if (prevCompletion?.passed) {
        const unlocksAt = m.cooldownDays ? addDays(prevCompletion.completedAt, m.cooldownDays) : null;
        available = !unlocksAt || unlocksAt <= now;
        if (!available) lock = { module: m, reason: "cooldown", unlocksAt };
      } else if (prevInSession && !(m.cooldownDays > 0)) {
        available = true;
      } else {
        available = false;
    ...
    if (isRetakeEligible(completion, m.retakeCooldownDays, now)) {
      playable.push(m);
      prevInSession = true;
    } else {
