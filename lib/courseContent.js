...
import { resolveRetryRules, retryGate } from "@/lib/retryPolicy";
...
/**
 * Пауза перед модулем: власна Module.cooldownDays, а якщо її не задано —
 * загальна для курсу Course.modulePauseDays («Розклад» у конструкторі,
 * 2026-09-17). Та сама логіка, що в lib/coursePlan.ts effectiveCooldown:
 * план у плеєрі і сама сесія мають бачити одну й ту саму паузу, інакше
 * план каже «відкриється через 3 дні», а плеєр пускає одразу.
 */
export function moduleCooldownDays(course, courseModule) {
...
export function computeModuleAvailability(course, completionsByModuleId) {
  ...
  for (let i = 0; i < modules.length; i++) {
    ...
    const cooldown = moduleCooldownDays(course, courseModule);
    const unlocksAt = cooldown ? addDays(prevCompletion.completedAt, cooldown) : null;
 ...
 */
function isRetakeEligible(completion, retakeCooldownDays, now, retryRules = null) {
  if (!completion) return true; // ще не проходив узагалі — це не "повторне"
  if (!completion.passed) {
    // Провалений модуль: діє «м'яке гальмо» перескладання, якщо його
    // налаштовано в конструкторі (lib/retryPolicy.ts). Без налаштувань —
    // повтор одразу, як і було.
    return retryRules ? retryGate(completion, retryRules, now).canRetryNow : true;
  }
  // Ідеальний результат покращувати нічим — перепроходження не пропонується
  // взагалі, незалежно від retakeCooldownDays (рішення користувача,
  // 2026-09-17, уже діє в lib/coursePlan.ts canRetake — тут той самий
  // guard бракував, і сесія плеєра/«Далі» на вступному екрані все одно
  // пропонували повторно пройти вже складений на 100% модуль, хоча картка
  // плану для нього жодної дії не показувала — реальна скарга користувача,
  // 2026-09-19).
  if (completion.scorePercent === 100) return false;
 ...
 * могло розсинхронитись зі справжнім вмістом). Сама формула — в
 * lib/estimateTime.js (без залежності від prisma, щоб її можна було
 * так само порахувати на клієнті — див. components/CoursePlayer.jsx).
 *
...
export function estimateModuleMinutes(courseModule) {
  ...
  return estimateMinutesFromComponentCount(componentCount);
}
 ...
 *
 * @param {{ modules: { id: number, title: string, order: number, cooldownDays: number|null, retakeCooldownDays?: number|null }[] }} course
 * @param {{ moduleId: number, passed: boolean, scorePercent: number, completedAt: Date }[]} moduleCompletions
 * @param {Date} [now] — момент розрахунку пауз перепроходження (тест підставляє свій)
 */
export function getModuleStatusList(course, moduleCompletions, now = new Date()) {
  ...
  return course.modules.map((courseModule) => {
    ...
    // Чи можна ПЕРЕПРОЙТИ модуль прямо зараз — той самий isRetakeEligible,
    // яким getSessionModules нижче вирішує, пускати в плеєр чи показати
    // методичку. Потрібно саме тут, бо картка курсу (components/CourseTile.jsx)
    // раніше рахувала свій підпис кнопки ЛИШЕ з балу курсу (pct !== 100 →
    // «Покращити результат») і нічого не знала про паузу перепроходження:
    // курс на 94%, складений сьогодні, обіцяв покращення, а відкривався
    // методичкою (скарга користувача 2026-09-19, курс «Тест графіка: 10
    // модулів» — єдиний модуль нижче 100% був ще під 2-денною паузою).
    // Один розрахунок на обидва місця, щоб вони не могли розійтись знову.
    const retakeCooldown = courseModule.retakeCooldownDays ?? RETAKE_COOLDOWN_DAYS_DEFAULT;
    const canRetakeNow = isRetakeEligible(completion, courseModule.retakeCooldownDays, now, resolveRetryRules(course, courseModule));
    return {
      id: courseModule.id,
      ...
      canRetakeNow,
      // Коли пауза спаде — лише для складеного нижче 100% під паузою
      // (решті нема що показувати: 100% не перепроходиться взагалі, а
      // доступне зараз і так доступне).
      retakeAvailableAt:
        completion?.passed && completion.scorePercent < 100 && !canRetakeNow && retakeCooldown
          ? addDays(completion.completedAt, retakeCooldown)
          : null,
    };
  });
}
...
/**
 * Модулі ОДНІЄЇ сесії плеєра. Раніше бралися лише модулі, чий попередник
 * уже СКЛАДЕНО в базі — тобто при першому відкритті курсу на 5 модулів
 * плеєр отримував лише перший, вважав його останнім і після нього
 * відправляв /submit усього курсу (курс ставав «completed» після одного
 * модуля; чекпоінт між модулями взагалі не показувався). Тепер:
 *  - модуль доступний, якщо попередній складено і пауза минула (як було),
 *    АБО попередній грається в цій же сесії і в цього модуля немає паузи
 *    (cooldownDays 0/null) — безперервне проходження з чекпоінтами;
 *  - складені модулі під паузою перепроходження пропускаються;
 *  - на першому модулі з паузою, чий попередник ще не складено, сесія
 *    зупиняється: nextLocked описує, чому і коли він відкриється.
 *
 * @returns {{ playable: Array, nextLocked: null | { module, reason: "pause"|"previous"|"cooldown", unlocksAt: Date|null } }}
 */
export function getSessionModules(course, completionsByModuleId, now = new Date()) {
  const playable = [];
  let nextLocked = null;
  ...
  for (let i = 0; i < course.modules.length; i += 1) {
    ...
    let available;
    let lock = null;
    if (i === 0) available = true;
    else {
      const prev = course.modules[i - 1];
      const prevCompletion = completionsByModuleId.get(prev.id);
      const cooldown = moduleCooldownDays(course, m);
      if (prevCompletion?.passed) {
        const unlocksAt = cooldown ? addDays(prevCompletion.completedAt, cooldown) : null;
        available = !unlocksAt || unlocksAt <= now;
        if (!available) lock = { module: m, reason: "cooldown", unlocksAt };
      } else if (prevInSession && !(cooldown > 0)) {
        available = true;
      } else {
        available = false;
        lock = prevInSession ? { module: m, reason: "pause", unlocksAt: null } : { module: m, reason: "previous", unlocksAt: null };
      }
    }
    if (!available) {
      nextLocked = lock;
    ...
    if (isRetakeEligible(completion, m.retakeCooldownDays, now, resolveRetryRules(course, m))) {
      playable.push(m);
      prevInSession = true;
    } else {
      // Складений і під паузою перепроходження — не в сесії, але наступний
      // за ним відкривається як після складеного (prevCompletion.passed).
      prevInSession = false;
    }
  }
  return { playable, nextLocked };
}
