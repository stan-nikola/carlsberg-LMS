 ...
 *
 * "Отримано" — складено ВСІ модулі курсу (`remainingCount === 0` — це вже
 * означає status "passed" у кожного). До 2026-09-19 тут стояла окрема,
 * вища планка «кожен модуль рівно на 100%», і в застосунку жило ТРИ різних
 * визначення сертифіката (тут, у списку «Досягнення» і в самому PDF-роуті)
 * — рішення користувача: скрізь один критерій, складений курс.
 */
function certificateStatus(plan: CoursePlan, certificateEnabled: boolean): { label: string; earned: boolean } | null {
  ...
  const earned = plan.remainingCount === 0;
  ...
  return { label: earned ? "Отримано" : "За складений курс", earned };
}
