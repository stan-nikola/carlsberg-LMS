...
vi.mock("@/lib/prisma", () => ({
  prisma: {
    ...
    enrollment: {
      ...
      // Реальний код тепер бере createManyAndReturn (щоб знати, КОМУ саме
      // створився Enrollment, і сповістити їх) — той самий шпигун, а
      // результат {count} перетворюється на масив рядків, як робить Prisma.
      createManyAndReturn: async (args) => {
        const r = await enrollmentCreateMany(args);
        const n = r?.count ?? args.data.length;
        return args.data.slice(0, n).map((d) => ({ employeeId: d.employeeId }));
      },
    },
  },
}));

vi.mock("@/lib/notifications", () => ({ notifyCourseAssigned: vi.fn() }));
vi.mock("@/lib/adminAuth", () => ({ SYSTEM_ADMIN_EXTERNAL_CODE: "SYSTEM-ADMIN" }));

const { assignCourseToPositionsAndTerritories, publishScheduledCourses, assignFirstLoginCourses } = await import(
...
describe("assignCourseToPositionsAndTerritories", () => {
  ...
  it("кидає помилку, коли порожні і positionCodes, і territoryIds, і employeeIds — нема кого шукати", async () => {
    await expect(
      assignCourseToPositionsAndTerritories(1, { positionCodes: [], assignedByUserId: 1 })
    ).rejects.toThrow("Provide positionCodes, territoryIds and/or employeeIds");
    expect(employeeFindMany).not.toHaveBeenCalled();
  });

  it("самі territoryIds без positionCodes — призначає всім, чий Employee.territoryId у списку (раніше не працювало: territoryIds був лише фільтром ВСЕРЕДИНІ пошуку по посаді і сам по собі нікого не знаходив)", async () => {
    courseFindUniqueOrThrow.mockResolvedValue({ id: 1, deadlineDays: null, isMandatory: true });
    employeeFindMany.mockResolvedValue([{ id: 878 }]);
    enrollmentCreateMany.mockResolvedValue({ count: 1 });

    await assignCourseToPositionsAndTerritories(1, { territoryIds: [136], assignedByUserId: 999 });

    expect(employeeFindMany).toHaveBeenCalledWith({ where: { territoryId: { in: [136] } }, select: { id: true } });
    const ids = enrollmentCreateMany.mock.calls[0][0].data.map((d) => d.employeeId);
    expect(ids).toEqual([878]);
  });
...
describe("publishScheduledCourses — targetPositions/publishAt самі по собі нічого не призначають", () => {
  ...
  it("курс без due publishAt (запит фільтрує lte:now) не потрапляє в обробку — findMany викликається з правильними умовами", async () => {
    ...
    // Курс публікується, якщо задано ХОЧ ОДНЕ з трьох: посади, території
    // АБО конкретні employeeIds (точкове призначення без жодної посади
    // теж має спрацювати — і так само курс лише з territoryIds, без
    // positionCodes, теж має тут зʼявитись).
...
describe("assignFirstLoginCourses", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("призначає лише курси з assignOnFirstLogin і від імені SYSTEM-ADMIN", async () => {
    courseFindMany.mockResolvedValue([{ id: 5, deadlineDays: 7, isMandatory: true }]);
    employeeFindFirst.mockResolvedValue({ id: 777, externalCode: "SYSTEM-ADMIN" });
    enrollmentCreateMany.mockResolvedValue({ count: 1 });

    const result = await assignFirstLoginCourses(42);

    // Вибірка саме за прапорцем — не "усі курси підряд".
    expect(courseFindMany).toHaveBeenCalledWith({ where: { assignOnFirstLogin: true } });
    expect(result.assignedCount).toBe(1);

    const payload = enrollmentCreateMany.mock.calls[0][0];
    expect(payload.data).toHaveLength(1);
    expect(payload.data[0].employeeId).toBe(42);
    expect(payload.data[0].courseId).toBe(5);
    // Курс призначає система, а не сам співробітник собі.
    expect(payload.data[0].assignedById).toBe(777);
    // Повторний перший вхід (чи повторний виклик) не має дублювати курс —
    // @@unique([employeeId, courseId]) плюс skipDuplicates.
    expect(payload.skipDuplicates).toBe(true);
  });

  it("без жодного курсу для новоприбулих не ходить ні в кого", async () => {
    courseFindMany.mockResolvedValue([]);

    const result = await assignFirstLoginCourses(42);

    expect(result.assignedCount).toBe(0);
    // Ні пошуку системного адміна, ні запису — зайвих запитів на КОЖНОМУ
    // першому вході бути не повинно.
