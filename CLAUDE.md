...
- **Сертифікат = СКЛАДЕНИЙ курс** (`Enrollment.passed`), один критерій на
  всі чотири місця: список у «Досягненнях» (`lib/achievements.ts`), PDF-роут
  (`app/api/courses/[slug]/certificate`), статус у плані курсу
  (`lib/coursePlan.ts certificateStatus`) і фінальний екран плеєра
  (`CompleteScreen` у `components/CoursePlayer.jsx`). До 2026-09-19 тут жило
  ТРИ різних визначення (100% по курсу / кожен модуль на 100% / passed), а
  плеєр до 2026-09-22 і далі показував кнопку лише за 100% (знайдено
  стендом механіки) — новий поріг додавати лише в усі місця одночасно. Реальний бал друкується
  в самому PDF, тому нижча планка сертифікат не знецінює.
