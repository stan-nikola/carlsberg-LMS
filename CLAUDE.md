...
- **Перескладання проваленого модуля (2026-09-17).** Дефолт лишається
  ...
  «Найскладніших модулях» як «у середньому спроб».
...
- **Сертифікат = СКЛАДЕНИЙ курс** (`Enrollment.passed`), один критерій на
  всі три місця: список у «Досягненнях» (`lib/achievements.ts`), PDF-роут
  (`app/api/courses/[slug]/certificate`) і статус у плані курсу
  (`lib/coursePlan.ts certificateStatus`). До 2026-09-19 тут жило ТРИ
  різних визначення (100% по курсу / кожен модуль на 100% / passed) —
  новий поріг додавати лише в усі три одночасно. Реальний бал друкується
