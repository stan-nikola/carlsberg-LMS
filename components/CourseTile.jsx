...
export function CourseTile({ course, enrollment, description, inProgressDescription, hasEmail = true }) {
  ...
  // Пройдений курс: методичка — ЛИШЕ за рівно 100% (тоді покращувати
  // нічого і сторінка курсу справді відкриє читальний режим). Будь-який
  // залік нижче 100% — «Покращити результат», навіть якщо пауза
  // перепроходження ще не минула: сторінка курсу тепер показує план з
  // датою, коли саме можна повернутись, а не мовчазну методичку
  // (скарга користувача, 2026-09-22: курс на 83% обіцяв методичку, хоча
  // модуль на 50% чекав на перескладання з 24.09). Незалік — пройти
  // знову (користувач, 2026-09-15).
  const enterLabel = isActuallyDone
    ? cs.passed
      ? cs.pct === 100 || (!canImproveNow && !nextRetakeAt)
        ? "Відкрити методичку"
        : "Покращити результат"
      : "Пройти курс знову"
    : trulyNotStarted
